...
To run and test the implemented processing components against the failure scenarios, the executable files of them are containerized and put in the same Docker compose environment along with th
To run the failure scenarios with each platform, Docker compose is used to containerize all components and run them locally in a Docker environment \cite{docker}. 

Regarding the platforms, while Apache Pulsar and NATS Streaming have official Docker images, Apache Kafka is not released with a Docker image. Nevertheless, there are a number of Kafka Docker images provided by third-party organizations. In the thesis, the Kafka Docker image from the company Confluent is used as it is regularly updated.

A Docker compose file is prepared for each platform. In each file, the Docker services for required infrastructure and applications are defined.
\begin{itemize}
	\item Infrastructure: the ESP platform in fault-tolerant configuration and the PostgreSQL relational database.
	\item Applications: three processing components: event generator, stream processor and view generator along with their corresponding failure-injected.
\end{itemize}
